...
def show_about(window, on_back):
    ...
    ttk.Label(frame, text=about_text, font=("Arial", 11), justify=LEFT).pack(pady=10, padx=30)

    links_frame = ttk.Frame(frame)
    links_frame.pack(pady=5)

    github_link = ttk.Label(links_frame, text="My GitHub", font=("Arial", 11, "underline"), bootstyle=PRIMARY, cursor="hand2")
    github_link.pack(side=LEFT, padx=10)
    github_link.bind("<Button-1>", lambda e: open_link("https://github.com/Winterlapse/wakassets"))

    forum_link = ttk.Label(links_frame, text="Forum Post", font=("Arial", 11, "underline"), bootstyle=PRIMARY, cursor="hand2")
